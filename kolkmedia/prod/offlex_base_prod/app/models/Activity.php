...
class Activity extends Eloquent
{
    ...
    public static function createClient($client, $notify = true)
    {
        ...
        $activity->message = Utils::encodeActivity(Auth::user(), 'heeft ', $client, ' aangegemaakt');
        $activity->save();
    ...
    public static function updateClient($client)
    {
        if ($client->is_deleted && !$client->getOriginal('is_deleted')) {
            ...
            $activity->message = Utils::encodeActivity(Auth::user(), 'heeft ', $client, ' verwijderd');
    ...
    public static function archiveClient($client)
    {
        if (!$client->is_deleted) {
            ...
            $activity->message = Utils::encodeActivity(Auth::user(), 'heeft ', $client, ' gearchiveerd');
            $activity->balance = $client->balance;
            $activity->save();
        }
    }
    ...
    public static function restoreClient($client)
    {
        ...
        $activity->message = Utils::encodeActivity(Auth::user(), 'heeft ', $client, ' hersteld');
        $activity->balance = $client->balance;
        $activity->save();
    }
    ...
    {
        if (Auth::check()) {
            $message = Utils::encodeActivity(Auth::user(), 'heeft ', $invoice, ' gemaakt');
        } else {
            $message = Utils::encodeActivity(null, 'heeft ', $invoice, ' gemaakt');
        }
    ...
    public static function archiveInvoice($invoice)
    {
        if (!$invoice->is_deleted) {
            ...
            $activity->message = Utils::encodeActivity(Auth::user(), 'heeft ', $invoice, ' gearchiveerd');
    ...
    public static function restoreInvoice($invoice)
    {
        ...
        $activity->message = Utils::encodeActivity(Auth::user(), 'heeft ', $invoice, ' hersteld');
        $activity->balance = $invoice->client->balance;
    ...
    public static function emailInvoice($invitation)
    {
        ...
        $activity->message = Utils::encodeActivity(Auth::check() ? Auth::user() : null, 'heeft ', $invitation->invoice, ' gemaild', $invitation->contact);
        $activity->balance = $client->balance;
        $activity->save();
    }
    ...
    public static function updateInvoice($invoice)
    {
        ...
        if ($invoice->is_deleted && !$invoice->getOriginal('is_deleted')) {
            ...
            $activity->message = Utils::encodeActivity(Auth::user(), 'heeft ', $invoice, ' verwijderd');
            $activity->balance = $invoice->client->balance;
            $activity->adjustment = $adjustment;
            $activity->save();
        } else {
            ...
            if ($diff != 0 || $fieldChanged) {
                ...
                $activity->message = Utils::encodeActivity(Auth::user(), 'heeft ', $invoice, ' bijgewerkt');
                $activity->balance = $client->balance;
    ...
    public static function viewInvoice($invitation)
    {
        ...
        $activity->message = Utils::encodeActivity($invitation->contact, 'heeft ', $invitation->invoice, ' bekeken');
    ...
    public static function approveQuote($invitation) {
        ...
        $activity->message = Utils::encodeActivity($invitation->contact, 'heeft ', $invitation->invoice , ' goedgekeurd');
        $activity->balance = $invitation->invoice->client->balance;
        $activity->save();
    }
    ...
    public static function createPayment($payment)
    {
        ...
        if ($payment->contact_id) {
            ...
            $activity->message = Utils::encodeActivity($payment->invitation->contact, 'heeft een '.trans('texts.'.$payment->getName()).' voor ', $payment->invoice, ' ingegeven');
        } else {
            $activity = Activity::getBlank($client);
            $message = $payment->payment_type_id == PAYMENT_TYPE_CREDIT ? 'heeft credit toegepast voor ' : 'heeft een  '.trans('texts.'.$payment->getName()).' voor ';
            $activity->message = Utils::encodeActivity(Auth::user(), $message, $payment->invoice, ' ingegeven');
        }
    ...
    public static function updatePayment($payment)
    {
        if ($payment->is_deleted && !$payment->getOriginal('is_deleted')) {
            ...
            //$activity->message = Utils::encodeActivity(Auth::user(), 'heeft  '.trans('texts.'.$payment->getName()). ' verwijderd' );
            $activity->message = Utils::encodeActivity(Auth::user(), 'heeft  een '.trans('texts.'.$payment->getName()). ' verwijderd' );
            $activity->balance = $client->balance;
    ...
    public static function archivePayment($payment)
    {
        ...
        $activity->message = Utils::encodeActivity(Auth::user(), 'heeft een '.trans('texts.'.$payment->getName()). ' gearchiveerd');
        $activity->balance = $client->balance;
    ...
    public static function restorePayment($payment)
    {
        ...
        $activity->message = Utils::encodeActivity(Auth::user(), 'heeft een '.trans('texts.'.$payment->getName()). ' hersteld');
        $activity->balance = $client->balance;
        $activity->adjustment = 0;
        $activity->save();
    }
    ...
    {
        $activity = Activity::getBlank();
        $activity->message = Utils::encodeActivity(Auth::user(), 'heeft '.Utils::formatMoney($credit->amount, $credit->client->currency_id).' credit aangemaakt');
    ...
    public static function updateCredit($credit)
    {
        if ($credit->is_deleted && !$credit->getOriginal('is_deleted')) {
            ...
            $activity->message = Utils::encodeActivity(Auth::user(), 'heeft '.Utils::formatMoney($credit->balance, $credit->client->currency_id).' credit verwijderd');
            $activity->balance = $credit->client->balance;
            $activity->save();
        } else {
            /*
    ...
    public static function archiveCredit($credit)
    {
        ...
        $activity->message = Utils::encodeActivity(Auth::user(), 'heeft '.Utils::formatMoney($credit->balance, $credit->client->currency_id).' credit gearchiveerd');
        $activity->balance = $credit->client->balance;
        $activity->save();
    }
    ...
    public static function restoreCredit($credit)
    {
        ...
        $activity->message = Utils::encodeActivity(Auth::user(), 'heeft '.Utils::formatMoney($credit->balance, $credit->client->currency_id).' credit hersteld');
        $activity->balance = $credit->client->balance;
        $activity->save();
    }
