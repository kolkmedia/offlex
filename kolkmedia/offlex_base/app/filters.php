...
App::before(function($request)
{
  ...
  if (!Utils::isOfflex() && !Utils::isDatabaseSetup())
  {
    return;
  }
  ...
  if (Auth::check())
  {
    ...
    if (!Utils::startsWith($_SERVER['REQUEST_URI'], '/news_feed') && !Session::has('news_feed_id')) {
      ...
      if (Utils::isOfflex()) {
        ...
        $file = @file_get_contents(OFFLEX_APP_URL . '/news_feed/' . Utils::getUserType() . '/' . OFFLEX_VERSION);
        $data = @json_decode($file);
      }      
      if ($data) {        
        if ($data->version != OFFLEX_VERSION) {
          $params = [
            'user_version' => OFFLEX_VERSION, 
            'latest_version'=> $data->version,
            'releases_link' => link_to(RELEASES_URL, 'Offlex', ['target' => '_blank'])
          ];
  ...
  if (!$claimingLicense && Input::has('license_key') && Input::has('product_id'))
  {
    ...
    $data = trim(file_get_contents((Utils::isOfflexDev() ? 'https://kolkmedia.offlex' : OFFLEX_APP_URL) . "/claim_license?license_key={$licenseKey}&product_id={$productId}"));
    ...
    else if ($productId == PRODUCT_WHITE_LABEL)
    {
      ...
      {
        $account = Auth::user()->account;
        $account->pro_plan_paid = OFFLEX_DATE;
        $account->save();
...
Route::filter('auth', function()
{
  if (Auth::guest()) 
  {
    if (Utils::isOfflex() || Account::count() == 0)
    {
...
Route::filter('api.access', function()
{
    ...
    $token = AccountToken::where('token', '=', Request::header('X-Offlex-Token'))->first(['id', 'user_id']);
