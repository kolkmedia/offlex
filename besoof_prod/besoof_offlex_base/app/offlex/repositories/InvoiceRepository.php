...
{
    public function getInvoices($accountId, $clientPublicId = false, $entityType = ENTITY_INVOICE, $filter = false, $overviewType = null)
    {
        ...
        if($overviewType != ENTITY_OVERVIEW){
    ...
    public function getDatatable($accountId, $clientPublicId = null, $entityType, $search, $overviewType = null)
    {
        $query = $this->getInvoices($accountId, $clientPublicId, $entityType, $search, $overviewType)
              ->where('invoices.is_quote', '=', $entityType == ENTITY_QUOTE ? true : false);
        
        if (\Session::get('filter_quarter:' .$overviewType)) {
        
        	//GET QUARTER DATE START TO END
        	$table = 'quarters';
        	$value = \Session::get('filter_quarter_value:' .$overviewType);
        	...
        	$query->whereBetween('invoices.invoice_date', array($start_date, $end_date))->get();
        }

        if (\Session::get('filter_year:' .$overviewType)) {
        	$value = \Session::get('filter_year_value:' .$overviewType);
        	$query->where(\DB::raw('YEAR(invoices.invoice_date)'), '=', $value);
        }
        
        if (\Session::get('filter_client:' .$overviewType)) {
        	$value = \Session::get('filter_client_value:' .$overviewType);
        	$query->where('invoices.client_id','=', $value);
        }
        
        $table = \Datatable::query($query);

        if (!$clientPublicId && $overviewType != ENTITY_OVERVIEW) {
        ...
        ->addColumn('dropdown', function ($model) use ($entityType,$overviewType) {
