...
@section('content')
    ...
    <div class="checkbox">
        {{ Former::checkbox('pdf_email_attachment',false)->raw() }}
        <label for="pdf_email_attachment">{{trans('texts.pdf_email_attachment')}} </label>
    </div>

    <p>&nbsp;</p>

    @if (Auth::user()->isPro())
        {{ Former::actions( Button::btn_info_submit(trans('texts.save'), array('class' => 'pull-right'))) }}
    @else
        <script>
            $(function () {
