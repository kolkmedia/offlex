...
class Utils
{
    ...
    public static function isOfflex()
    {
        return self::isOfflexProd() || self::isOfflexDev();
    }

    public static function isOfflexProd()
    {
        return isset($_ENV['OFFLEX_PROD']) && $_ENV['OFFLEX_PROD'];
    }

    public static function isOfflexDev()
    {
        return isset($_ENV['OFFLEX_DEV']) && $_ENV['OFFLEX_DEV'];
    }
    ...
    public static function getNewsFeedResponse($userType = false)
    {
        ...
        $response->version = OFFLEX_VERSION;
    ...
    public static function encodeActivity($person = null, $action, $entity = null, $action2=null, $otherPerson = null)
    {
        $person = $person ? $person->getDisplayName() : '<i>System</i>';
        $entity = $entity ? '['.$entity->getActivityKey().']' : '';
        $otherPerson = $otherPerson ? 'naar '.$otherPerson->getDisplayName() : '';
        $token = Session::get('token_id') ? ' ('.trans('texts.token').')' : '';
        return trim("$person $token $action $entity $action2 $otherPerson");
    }
    ...
    public static function decodeActivity($message)
    {
        ...
        if (count($matches) > 0) {
            $match = $matches[0];
            $type1 = trans('texts.'.$matches[1].'');
            $type = $matches[1];
            $publicId = $matches[2];
            $name = $matches[3];

            $link = link_to($type.'s/'.$publicId, $name);
            $message = str_replace($match, "$type1  $link", $message);
        }

        return $message;
    }
    public static function decodeLink($message)
    {
        $pattern = '/\[([\w]*):([\d]*):(.*)\]/i';
        preg_match($pattern, $message, $matches);
        ...
        if (count($matches) > 0) {
            ...
            $link = link_to($type.'s/'.$publicId, $name);
            $message = str_replace($match, "$type $link", $link);
        }
