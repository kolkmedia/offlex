...
{
  if (role === 'primary' && OFFLEX.primaryColor) {
    return setDocHexColor(doc, OFFLEX.primaryColor);
  } else if (role === 'secondary' && OFFLEX.secondaryColor) {
    return setDocHexColor(doc, OFFLEX.secondaryColor);
  }
...
function displaySubtotals(doc, layout, invoice, y, rightAlignTitleX)
{
  ...
  if (OFFLEX.parseFloat(invoice.custom_value1) && invoice.custom_taxes1 == '1') {
    data.push({'custom_invoice_label1': formatMoney(invoice.custom_value1, invoice.client.currency_id) })
  }
  if (OFFLEX.parseFloat(invoice.custom_value2) && invoice.custom_taxes2 == '1') {
...
function displayGrid(doc, invoice, data, x, y, layout, options)  {
  ...
  for (var i=0; i<data.length; i++) {
    ...
    if (typeof row === 'object') {
      ...
      } else if (key === 'discount' && OFFLEX.parseFloat(invoice.discount) && !parseInt(invoice.is_amount_discount)) {
...
function calculateAmounts(invoice) {
  ...
  for (var i=0; i<invoice.invoice_items.length; i++) {
    ...
    var lineTotal = roundToTwo(OFFLEX.parseFloat(item.cost)) * roundToTwo(OFFLEX.parseFloat(item.qty));
    if (tax) {
...
function displayInvoiceItems(doc, invoice, layout) {
  ...
  for (var i=0; i<invoice.invoice_items.length; i++) {
    ...
    var qty = OFFLEX.parseFloat(item.qty) ? roundToTwo(OFFLEX.parseFloat(item.qty)) + '' : '';
